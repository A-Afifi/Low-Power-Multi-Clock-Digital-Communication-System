...
# Created by write_sdc on Wed Oct 1 01:57:28 2025
...
create_generated_clock [get_pins UART_TX_ClkDiv/o_div_clk]  -name UART_TX_CLK  -source [get_pins UART_CLK_MUX/OUT]  -master_clock UART_CLK  -divide_by 32  -add
set_clock_uncertainty -setup 0.2  [get_clocks UART_TX_CLK]
set_clock_uncertainty -hold 0.1  [get_clocks UART_TX_CLK]
create_generated_clock [get_pins U_CLK_GATE/GATED_CLK]  -name ALU_CLK  -source [get_pins DFT_REF_MUX/OUT]  -master_clock REF_CLK  -divide_by 1  -add
set_clock_uncertainty -setup 0.2  [get_clocks ALU_CLK]
set_clock_uncertainty -hold 0.1  [get_clocks ALU_CLK]
create_clock [get_ports scan_clk]  -name DFTCLK  -period 100  -waveform {0 50}
...
set_input_delay -clock DFTCLK  20  [get_ports test_mode]
set_input_delay -clock DFTCLK  20  [get_ports SE]
set_input_delay -clock DFTCLK  20  [get_ports {SI[3]}]
set_input_delay -clock DFTCLK  20  [get_ports {SI[2]}]
set_input_delay -clock DFTCLK  20  [get_ports {SI[1]}]
set_input_delay -clock DFTCLK  20  [get_ports {SI[0]}]
set_output_delay -clock DFTCLK  20  [get_ports {SO[3]}]
set_output_delay -clock DFTCLK  20  [get_ports {SO[2]}]
set_output_delay -clock DFTCLK  20  [get_ports {SO[1]}]
set_output_delay -clock DFTCLK  20  [get_ports {SO[0]}]
set_output_delay -clock UART_TX_CLK  1736.29  [get_ports UART_TX_O]
set_clock_groups -asynchronous -name UART_CLK_1 -group [list [get_clocks UART_CLK] [get_clocks UART_TX_CLK]] -group [list [get_clocks REF_CLK] [get_clocks ALU_CLK]] -group [get_clocks DFTCLK]
